    ...
    fetch(this.url)
     ...
     .then((departamentos) => {
       ...
       departamentos.forEach((departamento) => {
         ...
         let { iddepartamento, descripcion, estado } = departamento;
         filas += ` <tr>
        <td>${iddepartamento}</td>
        <td>${descripcion}</td>
        <td>${estado ? "Activo" : "Inactivo"}</td>
        <td>
          <button type="button" class="btn btn-edit" id="btn-edit" data-id="${iddepartamento}">✏️</button>
          <button type="button" class="btn btn-delete" id="btn-delete" data-id="${iddepartamento}">❌</button>
        </td>
      </tr>
       `;
       });
  ...
  async eliminarDepartamento(id) {
    ...
    .then(async(empleados) =>{
      ...
      if (idcargo_empleado.length > 0) {
        alert("Este departamento esta asignado a un empleado por lo tanto no se puede eliminar")
      }else {
