  ...
  obtenerCargos() {
    fetch(this.url)
      ...
      .then((cargos) => {
        ...
        <td>
          <button type="button" class="btn btn-edit" id="btn-edit" data-id="${idcargo}">✏️</button>
          <button type="button" class="btn btn-delete" id="btn-delete" data-id="${idcargo}">❌</button>
        </td>
    ...
    fetch("http://localhost:3000/empleados")
      ...
      .then(async (empleados) => {
        ...
        if (idcargo_empleado.length > 0) {
          alert("Este cargo lo ocupa un empleado por lo tanto no se lo puede eliminar")
        } else {
